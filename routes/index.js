...
router.post("/shorten", async (req, res) => {
  ...
  if (!validUrl.isUri(longurl)) {
    return res.send("Yanlış Biçim Kullandınız: Lütfen Discord Sunucunuzun Linkini Koymayı Deneyin");
  }
...
router.post("/edit/:id", async (req, res) => {
  ...
  if (!validUrl.isUri(req.body.longurl)) {
    return res.json({
      message: "Yanlış Tür URL Lütfen Discord Sunucunuzun URL Girin"
    });
  }
  ...
  if (oldcustom) {
    return res.json({
      message:
        "Şu Anda Sitemizde Anlık Hata Oluşmaktadır Lütfen Siteden Çıkıp Tekrar Giriniz"
    });
  }
